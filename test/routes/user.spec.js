    ...
    describe('GET /user/{id}', () => {
        ...
        describe('when user is authenticated', () => {
            ...
            it('returns 1 user at a time', (done) => {
                ...
                server.inject(options, (response) => {
                    expect(response.result).to.have.property('name', payload.name);
                    expect(response.result).to.have.property('username', payload.username);
                    expect(response.result).to.have.property('email', payload.email);
                    expect(response.result).to.have.property('recoveryCode');
                    recoveryCode = response.result.recoveryCode;
                    expect(response.result).to.have.property('active', false);
                    done();
                });
            });
        ...
        describe('when a user verify his account', () => {
            ...
            it('returns 1 user at a time', (done) => {
                ...
                server.inject(options, (response) => {
                    ...
                    expect(response.result).to.have.property('username', payload.username);
                    expect(response.result).to.have.property('email', payload.email);
                    expect(response.result).to.have.property('recoveryCode', recoveryCode);
                    expect(response.result).to.have.property('active', true);
                    done();
                });
            });
        });
